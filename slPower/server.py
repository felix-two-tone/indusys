...
def loaddown():
  print ('Increasing load!')
  parameters = {'incomingLoad':'minus'}
  info = submitInformation(url,parameters);
  print(info.decode('utf-8'));
  return render_template('index.html')

@app.route('/chargemode/')
def chargemode():
  print ('Charge mode activated!')
  parameters = {'mode':'1'}
  info = submitInformation(url,parameters);
  print(info.decode('utf-8'));
  return render_template('index.html')

@app.route('/dischargemode/')
def dischargemode():
  print ('Discharge mode activated!')
  parameters = {'mode':'2'}
  info = submitInformation(url,parameters);
  print(info.decode('utf-8'));
  return render_template('index.html')
